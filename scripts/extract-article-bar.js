/**
 * Extract Article Bar from Figma
 */
  ...
  if (imageIds.length > 0) {
    ...
    try {
      ...
      if (imageResponse.images) {
        const outputDir = path.resolve(__dirname, '..', 'assets', 'figma', 'article-bar');
        ...
        for (const [imageId, imageUrl] of Object.entries(imageResponse.images)) {
          ...
          try {
            ...
            images.push({
              id: imageId,
              url: imageUrl,
              localPath: `./assets/figma/article-bar/${fileName}`,
            });
 ...
 * Main extraction
 */
async function extractArticleBar() {
  try {
    console.log('🔍 Fetching Figma file...');
    const fileResponse = await figmaRequest(`/files/${config.fileKey}`);
    
    // Find the article bar node (21-4431)
    const targetId = '21:4431';
    let barNode = findNodeById(fileResponse.document, targetId);
    
    // If not found, try searching by name
    if (!barNode) {
      function searchByName(node, name) {
        const nodeName = node.name ? node.name.toLowerCase() : '';
        if (nodeName.includes('article') && nodeName.includes('bar')) {
          return node;
        }
        if (node.children) {
          for (const child of node.children) {
      ...
      barNode = searchByName(fileResponse.document, 'article-bar');
    }
    
    if (!barNode) {
      throw new Error('Could not find article bar node');
    }
    
    console.log(`✅ Found article bar: ${barNode.name} (${barNode.id})`);
    ...
    const nodeFile = path.join(dataDir, 'article-bar-node.json');
    fs.writeFileSync(nodeFile, JSON.stringify(barNode, null, 2), 'utf8');
    console.log(`✅ Saved node data to ${nodeFile}`);
    
    // Extract images
    const images = await extractImages(barNode, targetId);
    
    // Save extraction summary
    const summary = {
      nodeId: barNode.id,
      nodeName: barNode.name,
      extractedAt: new Date().toISOString(),
      images: images,
    };
    
    const summaryFile = path.join(dataDir, 'article-bar-summary.json');
    fs.writeFileSync(summaryFile, JSON.stringify(summary, null, 2), 'utf8');
    
    console.log('\n✅ Article bar extraction complete!');
...
module.exports = { extractArticleBar };
