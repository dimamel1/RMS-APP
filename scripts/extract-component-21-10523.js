  ...
  if (imageIds.length > 0) {
    ...
    try {
      ...
      if (imageResponse.images) {
        const outputDir = path.resolve(__dirname, '..', 'assets', 'figma', 'component-21-10523');
        ...
        for (const [imageId, imageUrl] of Object.entries(imageResponse.images)) {
          ...
          try {
            ...
            images.push({
              id: imageId,
              url: imageUrl,
              localPath: `./assets/figma/component-21-10523/${fileName}`,
            });
...
async function extractComponent() {
  try {
    ...
    const nodeResponse = await figmaRequest(`/files/${config.fileKey}/nodes?ids=21:10523`);
    
    if (!nodeResponse.nodes || !nodeResponse.nodes['21:10523']) {
      throw new Error('Node not found');
    }
    
    const node = nodeResponse.nodes['21:10523'].document;
    console.log(`✅ Found component: ${node.name} (${node.id})`);
    ...
    const nodeFile = path.join(dataDir, 'component-21-10523-node.json');
    ...
    // Extract images
    const images = await extractImages(node, '21:10523');
    ...
    const summaryFile = path.join(dataDir, 'component-21-10523-summary.json');
